...
class TFModelWrapper(object):
    def __init__(self, name='rrea', lang='en_fr', **kwargs):
        ...
        default_params_dual_amn = dict(
            node_size=self.adj_features.shape[0],
        ...
        if name == 'dual_amn':
            default_params = default_params_dual_amn
            self.batch_size = 1024
        from .dual_amn.duala import get_trgat
        get_model = dict(rrea=self.get_trgat, mraea=get_mraea_model, dual_amn=get_trgat)
    ...
    def train1step(self, epoch=75):
        ...
        for i in range(epoch):
            ...
            if self.name == 'dual_amn':
                train_pair = train_set
                batch_size = self.batch_size
